<?php
/**
 * Regression guard for the N+1 fixes in AI-shared/optimization-plan-opus.md (item 2.4).
 *
 ...
 * command and not a CI job - see AI-shared/optimization-plan-opus.md section 10.
 */
...
class QueryBudgetCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int {
        ...
        $output->writeln('');
        if ($failed) {
            $output->writeln(sprintf('<error>%d scenario(s) scale with row count.</error> A per-row query came back - see AI-shared/optimization-plan-opus.md section A1.', $failed));
