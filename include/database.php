...
//  		DB::$ado->raiseErrorFn = $errh;
	...
	public static function CreateTable($name, $cols, $opts=false) {
		$dict = &self::dict();
		$arr = $dict->CreateTableSQL($name,$cols,isset($opts)?array_merge($opts,array('postgres'=>' WITH OIDS','mysql' => ' TYPE=ISAM')):array('postgres'=>' WITH OIDS','mysql' => ' TYPE=ISAM'));
	...
	public static function TypeControl($sql, & $arr) {
		if(SQL_TYPE_CONTROL) {		
			...
			$ret = str_replace(array('%s','%d','%f','%D','%T','%b'),'?',str_replace('%%','%',$sql));
		} 
		return $ret;
	}
...
$args[0] = self::TypeControl($sql,$inputarr);
...
$args = & func_get_args();
if(SQL_TIMES) $time = microtime(true);
$args[1] = self::TypeControl($sql,$inputarr);
