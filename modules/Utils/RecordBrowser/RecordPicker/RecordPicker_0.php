...
class Utils_RecordBrowser_RecordPicker extends Module {
	...
	public function body($tab, $element, $format, $crits=array(), $filters=array()) {
		...
		Libs_LeightboxCommon::display('leightbox_'.$element,
			$this->get_html_of_module($rb, array($element, $format, $crits, $filters), 'recordpicker'),
			$this->lang->t('Select'));
	}
	...
	public function create_open_href() {
		...
		return 'rel="leightbox_'.$this->element.'" class="lbOn button" onmousedown="init_all_rpicker_'.$this->element.'();"';
	}
}
