{* Was a <table> hand-wrapping every 4 label/data pairs into a new <tr> -
   flex-wrap replaces that: each .epesi-rbf-pair is a fixed-content flex
   item, and the container just wraps them onto as many per row as actually
   fit - no more hardcoded "4", and no x/first counter bookkeeping needed at
   all (same recipe as theme_adminlte/elements.tpl). *}
{$form_open}
