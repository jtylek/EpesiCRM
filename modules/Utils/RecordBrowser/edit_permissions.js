...
utils_recordbrowser__update_field_values = function (row, j) {
	var list = document.getElementById('crits_'+row+'_'+j+'_value');
	if (!list) return;
	for(i = (list.length-1); i >= 0; i--) {
		list.options[i] = null;
	}
	i = 0;
	selected_field = document.getElementById('crits_'+row+'_'+j+'_field').value;
...
utils_recordbrowser__update_field_sub_values = function (row, j) {
	var list = document.getElementById('crits_'+row+'_'+j+'_sub_value');
	if (!list) return;
	selected_field = document.getElementById('crits_'+row+'_'+j+'_field').value;
	selected_value = document.getElementById('crits_'+row+'_'+j+'_value').value;
	for(i = (list.length-1); i >= 0; i--) {
		list.options[i] = null;
	}
	if (utils_recordbrowser__field_sub_values[selected_field+'__'+selected_value]) {
		...
		document.getElementById('crits_'+row+'_'+j+'_sub_value').style.display='';
	} else {
		document.getElementById('crits_'+row+'_'+j+'_sub_value').style.display='none';
	}
}
...
utils_recordbrowser__init_clearance = function (current, max) {
	...
	if (utils_recordbrowser__clearance+1==utils_recordbrowser__clearance_max)
		document.getElementById('add_clearance').style.display = 'none';
	for (i=0; i<max; i++)
		document.getElementById('div_clearance_'+i).style.display = (i<=utils_recordbrowser__clearance)?'':'none';
}
utils_recordbrowser__add_clearance = function () {
	utils_recordbrowser__clearance++;
	if (utils_recordbrowser__clearance+1==utils_recordbrowser__clearance_max)
		document.getElementById('add_clearance').style.display = 'none';
	document.getElementById('div_clearance_'+utils_recordbrowser__clearance).style.display = '';
}
...
utils_recordbrowser__init_crits_and = function (current, max) {
	...
	for (i=0; i<max; i++)
		document.getElementById('div_crits_row_'+i).style.display = (i<=utils_recordbrowser__crits_ands)?'':'none';
}
utils_recordbrowser__add_and = function () {
	utils_recordbrowser__crits_ands++;
	if (utils_recordbrowser__crits_ands+1==utils_recordbrowser__crits_ands_max)
		document.getElementById('add_and').style.display = 'none';
	document.getElementById('div_crits_row_'+utils_recordbrowser__crits_ands).style.display = '';
}
...
utils_recordbrowser__init_crits_or = function (row, current, max) {
	...
	if (utils_recordbrowser__crits_ors[row]+1==utils_recordbrowser__crits_ors_max)
		document.getElementById('add_or_'+row).style.display = 'none';
	utils_recordbrowser__crits_ors_max = max;
	for (i=0; i<max; i++)
		document.getElementById('div_crits_or_'+row+'_'+i).style.display = (i<=utils_recordbrowser__crits_ors[row])?'':'none';
}
utils_recordbrowser__add_or = function (row) {
	utils_recordbrowser__crits_ors[row]++;
	if (utils_recordbrowser__crits_ors[row]+1==utils_recordbrowser__crits_ors_max)
		document.getElementById('add_or_'+row).style.display = 'none';
	document.getElementById('div_crits_or_'+row+'_'+utils_recordbrowser__crits_ors[row]).style.display = '';
}
