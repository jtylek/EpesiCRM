...
<div class="epesi-rv-row{if $f.type == 'multiselect'} multiselect_row{/if}{if $f.type == 'long text' || $f.type == 'file'} long_row{/if}">
    <div class="label{if $f.type == 'long text' || $f.type == 'file'} long_label{/if}{if $f.type == 'multiselect'} multiselect_label{/if}">{$f.label}{if $f.required}*{/if}{$f.advanced}</div>
