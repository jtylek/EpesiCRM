...
validate: function(ev,f) {
	...
	var car = Utils_PopupCalendarDatePicker.get_caret(elem);
	val = val.substring(0,car)+String.fromCharCode(key)+val.substring(car);
	Utils_PopupCalendarDatePicker.init_re(f);
	if(!Utils_PopupCalendarDatePicker.re.test(val))
		ev.stopPropagation();
	if(!Utils_PopupCalendarDatePicker.re.test(elem.value)) {
...
init_re: function(f) {
	if(Utils_PopupCalendarDatePicker.format!=f) {
		Utils_PopupCalendarDatePicker.re = new RegExp();
//		alert(Utils_PopupCalendarDatePicker.format2regexp(f));
		Utils_PopupCalendarDatePicker.re.compile('^'+Utils_PopupCalendarDatePicker.format2regexp(f)+'$');
		Utils_PopupCalendarDatePicker.format=f;
	}
},
