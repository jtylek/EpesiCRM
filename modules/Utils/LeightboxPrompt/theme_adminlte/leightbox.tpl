...
{* Rendered inside a Leightbox overlay (Libs_LeightboxCommon::display(), already
   AdminLTE-skinned on its own - modules/Libs/Leightbox/theme_adminlte). This is
   the generic Utils_LeightboxPrompt chooser grid any module can add_option()s
   into (e.g. CRM_Calendar's "New Event" -> Phonecall/Task/Meeting/...), so it
   can't assume a fixed button count - a fluid flex-wrap grid instead of the
   legacy version's row-of-6-then-wrap <table>. {$b.open}/{$b.close} are raw
   '<a ...>'/'</a>' strings built in LeightboxPrompt_0.php::body(), landing
   directly as .epesi-lbp-grid's children. *}
