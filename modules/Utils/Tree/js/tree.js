	...
	utils_tree_expand_all = function(id, sub) {
		for( i = 0; i < sub; i++) {
			is_collapsed[id][i] = 0;
			if(document.getElementById('utils_tree_'+id+'_'+i)) {
	...
	tree_node_visibility_toggle = function( id, sub ) {
		if(document.getElementById('utils_tree_'+id+'_'+sub)) {
			if( is_collapsed[id][sub] == 0 ) {
				is_collapsed[id][sub] = 1;
				document.getElementById('utils_tree_'+id+'_'+sub).style.display = "none";
				document.getElementById('utils_tree_opener_img_'+id+'_'+sub).src = "modules/Utils/Tree/theme/opener_active_closed.gif";
			} else {
				is_collapsed[id][sub] = 0;
				document.getElementById('utils_tree_'+id+'_'+sub).style.display = "block";
				document.getElementById('utils_tree_opener_img_'+id+'_'+sub).src = "modules/Utils/Tree/theme/opener_active_open.gif";
			}
		}
	}
