...
class Utils_GenericBrowser extends Module {
	...
	public function body($template=null,$paging=true){
		...
		foreach($this->columns as $v) {
			...
			if (!Base_User_SettingsCommon::get('Utils/GenericBrowser','adv_history') && $v['name']==$order[0]['column']) $sort = '<img width="8" height="8" style="vertical-align: middle;" border="0" src="'.Base_ThemeCommon::get_template_file('Utils_GenericBrowser','sort-'.strtolower($order[0]['direction']).'ending.png').'" />';
			else $sort = '';
			$headers[$col_pos[$i]['pos']]['label'] .= isset($v['order'])?'<a '.$this->create_unique_href(array('change_order'=>$v['name'])).'>'.$v['name'] . '&nbsp;' . $sort . '</a>':$v['name'];
