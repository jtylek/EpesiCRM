...
class Utils_GenericBrowser extends Module {
	...
	private $font_awesome_translate = array(
		'view' => 'folder-open',
		'delete' => 'trash',
		'edit' => 'pencil',
		'info' => 'info',
		'restore' => 'backward',
		'append data' => 'paperclip',
		'active-on' => 'circle',
		'active-off' => 'circle-o',
		'history' => 'history',
		'history_inactive' => 'history',
		'print' => 'print',
		'move-down' => 'arrow-down',
		'move-up' => 'arrow-up',
		'move-up-down' => 'arrows-v'
	);
  ...
  '<button type="button" class="btn btn-default dropdown-toggle btn-xs" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="border-radius: 65px">'.
    '<span class="glyphicon glyphicon-option-vertical"></span></button><div class="dropdown-menu-container"><ul class="dropdown-menu" style="padding: 5px 0">';
			...
			foreach ($r as $k => $v) {
				...
				$col[$k]['attrs'] .= ' class="Utils_GenericBrowser__td Utils_GenericBrowser__td_data_rows' . (isset($v['class']) ? $v['class'] : '') . '"';
		...
		$letter_links = null;
		if (isset($quickjump)) {
			$options['letter_links'] = $this->get_quickjump_letters();
		...
		$options['cols_width_id'] = $this->columns_width_id;
