		...
		if($this->is_back()) {
			...
			if($form->validate()) {
				...
				if(isset($id)) {
					...
					if(isset($r['is_deadline']) && $r['is_deadline'])
						DB::Execute('INSERT INTO utils_tasks_task(title,description,permission,priority,status,longterm,deadline,created_by,created_on,page_id,parent_module) VALUES (%s,%s,%d,%d,%d,%b,%D,%d,%T,%s,%s)',array($r['title'],$r['description'],$r['permission'],$r['priority'],$r['status'],isset($r['longterm']) && $r['longterm'],$r['deadline'],Acl::get_user(),time(),$this->mid,$this->get_type()));
					else
