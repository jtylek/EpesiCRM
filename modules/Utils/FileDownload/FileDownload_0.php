...
class Utils_FileDownload extends Module {
	...
	public function body($file=null,$callback=null) {
		...
		print('<div id="'.$id.'"></div>');
		eval_js_once('utils_filedownload_refresh = function(id,path){var stat=document.getElementById(id);if(!stat || stat.innerHTML==\'Processing downloaded file\') return;'.
			'jQuery(\'#\'+id).load(\''.$this->get_module_dir().'refresh.php\',{path: path});'.
		...
		print('<iframe src="'.$this->get_module_dir().'download.php?'.http_build_query(array('client_id'=>defined('CID') ? CID : false,'path'=>$path)).'"  width=0 height=0 frameborder=0>');
