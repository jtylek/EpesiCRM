...
class Utils_CalendarCommon extends ModuleCommon {
	public static function print_event($ev,$mode='',$with_div=true) {
		...
		if(!isset($ev['move_action']) || $ev['move_action']===true)
			$th->assign('move_href', Utils_PopupCalendarCommon::create_href('move_event'.str_replace(array('#','-'),'_',$ev['id']), $link_text,null,null,'jQuery(popup).clonePosition(document.getElementById(\'utils_calendar_event:'.$ev['id'].'\'),{setWidth:false,setHeight:false,offsetTop:document.getElementById(\'utils_calendar_event:'.$ev['id'].'\').offsetHeight})'));
