...
class Utils_Calendar_View_Week extends Module {
	private $date;
			...
			if(isset($events[$day]) && is_array($events[$day])) {
				...
				foreach($events[$day] as $key=>$EV) {
					...
					if($this->logged > 0) {
						...
						// delete
						if($EV['access'] == 0 || $EV['created_by'] == $this->logged)
							$more .= '<a '.$this->parent->create_confirm_callback_href('Are you sure, you want to delete this event?', array($this, 'delete_event'), array($module, $EV['id'])).' class=icon><img style="vertical-align: middle;" border="0" width="14" height="14" src='.Base_ThemeCommon::get_template_file('CRM_Calendar', 'delete.png').'></a> ';
