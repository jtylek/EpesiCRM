<table width="80%" class="utils_bookmarkbrowser_browser">
	<tr>
		{foreach from=$list key=cap item=bm}
			<td width={$header_width}% class=utils_bookmarkbrowser_tab><div onmouseover="utils_bookmarkbrowser_markTab(this)"  onmouseout="utils_bookmarkbrowser_unmarkTab(this)" class=utils_bookmarkbrowser_tab onclick="utils_bookmark_goto('{$content_id}', 'bkmk_{$bm}')"><b>{$bm}</b></div></td>
		{/foreach}
	</tr>
	<tr>
