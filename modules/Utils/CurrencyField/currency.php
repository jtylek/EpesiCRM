...
class HTML_QuickForm_currency extends HTML_QuickForm_input {
	...
	function toHtml() {
		...
		if ($this->_flagFrozen) {
			...
			eval_js('jQuery(document.getElementById(\''.$id.'\')).on(\'keypress\',Utils_CurrencyField.validate.bind(Utils_CurrencyField))');
			eval_js('jQuery(document.getElementById(\''.$id.'\')).on(\'blur\',Utils_CurrencyField.validate_blur.bind(Utils_CurrencyField))');
		}
		return $str;
	} //end func toHtml
	...
	function setValue($value) {
		...
		$this->updateAttributes(array('value'=>str_replace('.',Utils_CurrencyFieldCommon::get_decimal_point($val[1] ?? $this->currency),$val[0] ?? '')));
