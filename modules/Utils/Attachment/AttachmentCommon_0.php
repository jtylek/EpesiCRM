...
class Utils_AttachmentCommon extends ModuleCommon {
	...
	public static function bootstrap_icon() { return 'bi-journal'; }
	...
	// Thin wrapper kept for CRM_Roundcube's existing call site - the actual logic
	// (nothing Attachment-specific about it) now lives on Utils_FileStorageCommon,
	// shared with any other caller that just needs a public unauthenticated link
	// for a stored file. $label isn't persisted - the table has no column for it.
	public static function create_remote($file_id, $label, $expires) {
		return Utils_FileStorageCommon::create_remote_link($file_id, $expires);
	}
