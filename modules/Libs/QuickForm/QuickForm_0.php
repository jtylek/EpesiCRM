...
//			if($func_name==='accept') trigger_error(print_r($args,true));
	...
	public function get_submit_form_js_by_name($form_name, $submited, $indicator, $queue=false) {
		...
		foreach ($form_name as $f) {
            ...
            if ($submited) {
				$pre .= "document.getElementById('".addslashes($f)."').submited.value=1;";
			}
			$pre .= "jQuery(document).trigger('e:submit_form','".$f."');";
			$pre .= str_replace('this',"document.getElementById('".addslashes($f)."')",Libs_QuickFormCommon::get_on_submit_actions());
			if ($chj) $chj .= "+'&'+";
			$chj .= "jQuery(document.getElementById('".addslashes($f)."')).serialize()";
			if ($submited) $post .= "document.getElementById('".addslashes($f)."').submited.value=0;";
		}
	...
	public function assign_theme($name, & $theme, &$renderer=null){ 
		...
		// guard (Smarty dot-notation compiles straight to $form_data['header'],
		// see [[report-all-errors-exits-on-warning]]) - toArray() only sets this
		// key when the form actually has header-type elements.
		$form_data['header'] ??= array();
	...
	public function get_element_by_array(array $v, & $default_js = null) {
		...
		switch($v['type']){
			...
			case 'multiselect':
				...
				$default_js .= 'e = document.getElementById(\''.$this->getAttribute('name').'\').'.$v['name'].'__from;'.
			...
			case 'checkbox':
				...
				$default_js .= 'document.getElementById(\''.$this->getAttribute('name').'\').'.$v['name'].'.checked = '.($v['default']?1:0).';';
				break;
			...
			case 'textarea':
				...
				$default_js .= 'document.getElementById(\''.$this->getAttribute('name').'\').'.$v['name'].'.value = \''.$v['default'].'\';';
				break;
