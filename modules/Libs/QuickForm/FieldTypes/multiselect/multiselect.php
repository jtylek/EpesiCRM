    ...
    function toHtml()
    {
        ...
        if ($this->_flagFrozen) {
            ...
            foreach ($this->_options as $k=>$option) {
                $selected = (is_array($this->_values) && in_array((string)$option['attr']['value'], $this->_values))?'selected':'';
                $options_ .= $tabs . "\t<option " . $this->_getAttrString($this->_options[$k]['attr']) . " ".$selected.">" . $this->_options[$k]['text'] . "</option>\n";
            }
