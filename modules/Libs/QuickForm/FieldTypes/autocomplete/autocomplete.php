...
class HTML_QuickForm_autocomplete extends HTML_QuickForm_text {
    ...
    function toHtml() {
        if ($this->_flagFrozen) {
			...
			load_js('modules/Libs/QuickForm/FieldTypes/autocomplete/autocomplete.js');
			eval_js('var epesi_autocompleter = new EpesiAutocompleter(\''.$id.'\', \''.$id.'_suggestbox\', \'modules/Libs/QuickForm/FieldTypes/autocomplete/autocomplete_update.php?'.http_build_query(array('cid'=>defined('CID') ? CID : false, 'key'=>$key)).'\', {frequency: 0.6});');
