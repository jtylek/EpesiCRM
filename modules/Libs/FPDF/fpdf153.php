...
function Table($header,$data, $w)
{
	//Header
	foreach($header as $i=>$col)
		$this->Cell($w[$i],7,$col,1);
	$this->Ln();
	//Data
	foreach($data as $row)
	{
...
//Colored table
function FTable($header,$data, $w){
	//Colors, line width and bold font
	$this->SetFillColor(200,200,200);
	$this->SetFont('','B');
	//Header
	$this->FTable_row($header,$w);
	...
	$fill=0;
	foreach($data as $row) {
		$this->FTable_row($row,$w);
//		$this->MultiCell($w[0],6,$row[0]['value'],1,'J');
		$this->Cell(array_sum($w),0,'','T');
		$this->Ln();
	    $fill=!$fill;
	    if (!$fill){
	...
	$this->Cell(array_sum($w),9,'','');
	$this->Ln();
}

private function FTable_row($row,$w){
	$cw = & $this->CurrentFont['cw'];
	$max_lines_no = 1;
	$wmax = array();
	$length = 0;
	for($i=0;$i<count($row);$i++){
		$line[$i] = array();
		$wmax[$i] = ($w[$i]-2*$this->cMargin)*1000/$this->FontSize;
		$last_cut = 0;
		$lines = 0;
		$length = 0;	
		for ($l=0;$l<strlen($row[$i]['value']);$l++){
			$length += $cw[$row[$i]['value']{$l}];
			if ($length>$wmax[$i]) {
				$cut_to = $l;
				...
				$text = substr($row[$i]['value'],$last_cut,$cut_to-$last_cut);
				if ($row[$i]['value']{$cut_to}!=' ') $text .= '-';
				$line[$i][] = $text;
//				print('ASSIGN: $line['.$i.']['.$lines.']='.$line[$i][$lines].'<br>');
				$lines++;
				$l = $last_cut = $cut_to;
				$length = 0;
			} elseif ($l == strlen($row[$i]['value'])-1) {
				$line[$i][] = substr($row[$i]['value'],$last_cut,$l-$last_cut+1);
//				print('ASSIGN LAST: $line['.$i.']['.$lines.']='.$line[$i][$lines].'<br>');
				$lines++;
			}
		}
		if ($lines>$max_lines_no) $max_lines_no = $lines;
	}
	$this->Ln();
	for($j=0;$j<$max_lines_no;$j++){
		for($i=0;$i<count($row);$i++){
