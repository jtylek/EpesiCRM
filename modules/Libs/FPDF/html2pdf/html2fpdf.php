...
if (!defined('RELATIVE_PATH')) define('RELATIVE_PATH','modules/Libs/FPDF/html2pdf/');
...
//! @desc The header is printed in every page.
  ...
  {
    $y = $this->y;
    if (!is_array($content)) trigger_error(print_r($content,true));
    foreach($content as $tableheader)
    {
      ...
      $border = isset($tableheader['border'])?$tableheader['border']:'';
      $align = $tableheader['a'];
      //Align
      $this->divalign=$align;
			$this->x = $x;
...
// What this gets: < $tag $attr['WIDTH']="90px" > does not get content here </closeTag here>
  ...
  $this->blockjustfinished=false;
    ...
    case 'ADDRESS': 
      ...
      {
   			$this->cssbegin=true;
 				if (isset($attr['CLASS']) && isset($this->CSS[$attr['CLASS']])) $properties = $this->CSS[$attr['CLASS']];
    ...
    for( $j = 0 ; $j < $numcols ; $j++ ) //Columns
    {
  			if (isset($cells[$i][$j]) && $cells[$i][$j])
        {
          ...
          if ($this->usetableheader and $i == 0 )
          {
              ...
              $tableheader[$j]['textbuffer'] = isset($cell['textbuffer'])?$cell['textbuffer']:'';
              $tableheader[$j]['a'] = isset($cell['a'])? $cell['a'] : 'L';
              $tableheader[$j]['va'] = isset($cell['va'])?$cell['va']:'';
              $tableheader[$j]['mih'] = $cell['mih'];
              $tableheader[$j]['bgcolor'] = $fill;
              if (isset($table['border']) && $table['border']) $tableheader[$j]['border'] = 'all';
