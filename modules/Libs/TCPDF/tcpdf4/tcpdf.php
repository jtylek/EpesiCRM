	...
	*/
	class TCPDF {
		...
		*/
		...
		/**
		 * Array used to store footer positions inside the pages buffer.
		 * @access protected
		...
		public function Footer() {
			if ($this->print_footer) {
				...
				if ($this->barcode) {
					...
					$this->write1DBarcode($this->barcode, "C128B", $this->GetX(), $footer_y + $line_width, $barcode_width, $footer_height - $line_width, 0.3, '', '');	
				}
		...
		public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false) {	
			...
			if ($endpage > $startpage) {
				...
				for ($page=$startpage; $page <= $endpage; $page++) {
					...
					$this->SetX($x);
					$ccode = $this->FillColor."\n".$this->getCellCode($w, $h, "", $border, 1, '', $fill);
					if ($border OR $fill) {
	 ...
	 /**
		...
		*/
		protected function _textstring($s) {
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType')) {
		...
		* @param array $coords <ul><li>for one patch mesh: array(float x1, float y1, .... float x12, float y12): 12 pairs of coordinates (normally from 0 to 1) which specify the B�zier control points that define the patch. First pair is the lower left edge point, next is its right control point (control point 2). Then the other points are defined in the order: control point 1, edge point, control point 2 going counter-clockwise around the patch. Last (x12, y12) is the first edge point's left control point (control point 1).</li><li>for two or more patch meshes: array[number of patches]: arrays with the following keys for each patch: f: where to put that patch (0 = first patch, 1, 2, 3 = right, top and left of precedent patch - I didn't figure this out completely - just try and error ;-) points: 12 pairs of coordinates of the B�zier control points as above for the first patch, 8 pairs of coordinates for the following patches, ignoring the coordinates already defined by the precedent patch (I also didn't figure out the order of these - also: try and see what's happening) colors: must be 4 colors for the first patch, 2 colors for the following patches</li></ul>
				...
				if (preg_match($tagpattern, $element)) {
					...
					if ($element{0} == '/') {
						...
						if (($dom[$key]['value'] == "tr") AND (!isset($dom[($dom[($dom[$key]['parent'])]['parent'])]['cols']))) {
							if (!isset($dom[($dom[$key]['parent'])]['cols'])) $dom[($dom[$key]['parent'])]['cols']=1;
						...
						if (($dom[$key]['value'] == "th") OR ($dom[$key]['value'] == "td")) {
							...
							if (!isset($dom[($dom[$key]['parent'])]['cols'])) $dom[($dom[$key]['parent'])]['cols'] = 0;
					...
					if ($dom[$key]['opening']) {	
						...
						if (($dom[$key]['value'] == "td") OR ($dom[$key]['value'] == "th")) {
							...
							if (isset($dom[$key]['width'])) {
								...
								if (!isset($dom[$table_el]['cols'])) $dom[$table_el]['cols']=1;
								$cellw = ($colspan * ($table_width / $dom[$table_el]['cols']));
							}
							$cellw -= $cellspacing;
							if (!isset($dom[$key]['content'])) $dom[$key]['content']='';
							...
							if (isset($dom[$parentid]['attribute']['rowspan'])) {
								...
								if ($rowspan > 1) {
									$trsid = array_push($dom[$table_el]['rowspans'], array('rowspan' => $rowspan, 'colspan' => $colspan, 'startpage' => $this->page, 'startx' => $this->x, 'starty' => $this->y, 'intmrkpos' => strlen($this->pages[$this->page])));
								}
							}	else {
								$rowspan = 1;
							}
							...
							if (isset($dom[$table_el]['rowspans'])) {
								foreach ($dom[$table_el]['rowspans'] as $k => $trwsp) {
									if  (($trwsp['startx'] == $this->x) AND ($trwsp['starty'] < $this->y) AND ($trwsp['rowspan'] > 1)) {
							...
							if ($endrwsp >= 0) {
								$dom[$trid]['endy'] = max($dom[$table_el]['rowspans'][$endrwsp]['endy'], $dom[$trid]['endy']);
								$dom[$trid]['endpage'] = max($dom[$table_el]['rowspans'][$endrwsp]['endpage'], $dom[$trid]['endpage']);
							}
							$this->x += ($cellspacingx / 2);
						} else {
