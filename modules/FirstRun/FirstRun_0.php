...
class FirstRun extends Module {
	...
	public function body() {
		...
		ob_start();
		print('<center>');
		if(ModuleManager::is_installed('Base')>=0) {
			$post_install = $this->get_module_variable('post-install',array());
			foreach($post_install as $i=>$v) {
				$v = str_replace('/','_',$v);
				ModuleManager::include_install($v);
				$f = array($v.'Install','post_install');
				$fs = array($v.'Install','post_install_process');
				...
				$form->addElement('header',null,'Post installation of '.str_replace('_','/',$v));
				$form->add_array($ret);
	...
	public function done($d) {
		...
		$this->set_module_variable('post-install',$pkgs);
		location();
	}
