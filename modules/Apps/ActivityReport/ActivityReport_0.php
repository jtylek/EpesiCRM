...
class Apps_ActivityReport extends Module {
		...
		if (!empty($tables)) {
			...
			while ($row=$ret->FetchRow()) {
				...
				switch ($row['action']) {
					case 'edit': 	$details = DB::GetAssoc('SELECT field, old_value FROM '.$row['tab'].'_edit_history_data WHERE edit_id=%d', array($row['id']));
						if (isset($details['id'])) {
							...
							$action = __('Edited');
							$action = '<a '.Utils_TooltipCommon::tooltip_leightbox_mode().' '.Utils_TooltipCommon::ajax_open_tag_attrs(array('Utils_RecordBrowserCommon', 'get_edit_details_label'), array($row['tab'], $row['r_id'], $row['id']), 500, true, true).'>'.$action.'</a>';
						}
						$r_id = $row['r_id'];
						break;
				...
				if (!Utils_RecordBrowserCommon::get_access($row['tab'], 'view', Utils_RecordBrowserCommon::get_record($row['tab'], $r_id))) {
					...
					$link = Utils_TooltipCommon::icon(Utils_RecordBrowserCommon::get_html_record_info($row['tab'], $r_id), false);
					$link .= '&nbsp;';
	...
	public function contact_addon($r)
	{
		if (!Apps_ActivityReportCommon::has_access_to_report($r)) {
			return;
		}
		$this->body($r);
	}
