...
{* Multiselect and long text fields, in their shared RecordBrowser field
   order (RecordBrowser_0.php's $secondary_blocks: consecutive multiselect
   fields grouped into one column-balanced .epesi-rv-columns block, each long
   text field its own full-width .longfields block) - so moving a long text
   field above/below a multiselect field in Manage Fields is respected here
   instead of long text always trailing every multiselect. *}
