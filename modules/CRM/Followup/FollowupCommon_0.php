	...
	public static function drawLeightbox($prefix) {
		...
		if (!isset(self::$leightbox_ready[$prefix])) {
			...
			if ($meetings) {
				...
				eval_js('jQuery(document.getElementById(\''.$prefix.'_new_meeting_button\')).on(\'click\', '.$prefix.'_followups_deactivate)');
			}
			...
			if ($tasks) {
				...
				eval_js('jQuery(document.getElementById(\''.$prefix.'_new_task_button\')).on(\'click\', '.$prefix.'_followups_deactivate)');
			}
			...
			if ($phonecall) {
				...
				eval_js('jQuery(document.getElementById(\''.$prefix.'_new_phonecall_button\')).on(\'click\', '.$prefix.'_followups_deactivate)');
			}
			...
			eval_js('jQuery(document.getElementById(\''.$prefix.'_just_close_button\')).on(\'click\', '.$prefix.'_followups_deactivate)');

			eval_js($prefix.'_submit_form = function () {'.
						'document.getElementById(\''.$prefix.'_follow_up_form\').submited.value=1;Epesi.href(jQuery(document.getElementById(\''.$prefix.'_follow_up_form\')).serialize(), \'processing...\');document.getElementById(\''.$prefix.'_follow_up_form\').submited.value=0;'.
					'}');
			...
			eval_js($prefix.'_set_id = function (id) {'.
						...
						'document.getElementById("'.$prefix.'_closecancel").value=3;'.
