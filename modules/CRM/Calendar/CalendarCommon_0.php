...
/*		$emp = array();
		...
		if($action == 'new') {
			...
			$form->addElement('select','messenger_before',$this->t('Popup alert'),array(0=>$this->t('on event start'), 900=>$this->t('15 minutes before event'), 1800=>$this->t('30 minutes before event'), 2700=>$this->t('45 minutes before event'), 3600=>$this->t('1 hour before event'), 2*3600=>$this->t('2 hours before event'), 3*3600=>$this->t('3 hours before event'), 4*3600=>$this->t('4 hours before event'), 8*3600=>$this->t('8 hours before event'), 12*3600=>$this->t('12 hours before event'), 24*3600=>$this->t('24 hours before event')));
			$form->addElement('textarea','messenger_message',$this->t('Popup message'), array('id'=>'messenger_message'));
			$form->addElement('select','messenger_on',$this->t('Alert'),array('none'=>$this->t('None'),'me'=>$this->t('me'),'all'=>$this->t('all selected employees')),array('onChange'=>'crm_calendar_event_messenger(this.value!="none");$("messenger_message").value=$("event_title").value;'));
...
//		print('='.$form->exportValue('recurrence').'=');
		...
		$form->addElement('select','recurrence_interval',$this->t('Recurrence interval'),array('everyday'=>$this->t('everyday'),'second'=>$this->t('every second day'),'third'=>$this->t('every third day'),'fourth'=>$this->t('every fourth day'),'fifth'=>$this->t('every fifth day'),'sixth'=>$this->t('every sixth day'),'week'=>$this->t('once every week'),'week_custom'=>$this->t('customize week'),'two_weeks'=>$this->t('every two weeks'),'month'=>$this->t('every month'),'year'=>$this->t('every year')),array('onChange'=>'crm_calendar_event_recurrence_custom(this.value=="week_custom")', 'id'=>'recurrence_interval'));
