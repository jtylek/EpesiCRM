	...
	public function applet($conf,$opts) {
		...
		foreach($ret as $row) {
			...
			if($row['description'])
				...
				$title = $row['title'];
			$gb->add_row($view_action.((!isset($ex['timeless']) || !$ex['timeless'])?Utils_TooltipCommon::create($ex['start'],$l->t('Duration: %s<br>End: %s',array($ex['duration'],$ex['end']))):$ex['start']).'</a>',$view_action.$title.'</a>');
		}
