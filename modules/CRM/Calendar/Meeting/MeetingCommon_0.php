...
class CRM_Calendar_MeetingCommon extends Utils_Calendar_EventCommon {
	...
	public static function get($id) {
		$row = DB::Execute('SELECT start,end,title,description,id,timeless,priority,created_by,created_on,edited_by,edited_on FROM crm_calendar_meeting_event WHERE id=%d AND '.self::$filter,array($id))->FetchRow();
		$result = array();
		if ($row) {
...
//		print('SELECT start,end,title,description,id,timeless,priority,created_by,created_on,edited_by,edited_on FROM crm_calendar_meeting_event WHERE ((start>=%d AND start<%d) OR (end>=%d AND end<%d)) AND '.self::$filter);
		$ret = DB::Execute('SELECT start,end,title,description,id,timeless,priority,created_by,created_on,edited_by,edited_on FROM crm_calendar_meeting_event WHERE ((start>=%d AND start<%d) OR (end>=%d AND end<%d)) AND '.self::$filter,array($start,$end,$start,$end));
		$result = array();
