...
class CRM_ProjectPlanner_ProjectEventCommon extends Utils_Calendar_EventCommon {
	...
	public static function get($id) {
		$result = DB::GetRow('SELECT \'blue\' as color,start,end,employee_id,id,0 as timeless FROM crm_projectplanner_work WHERE id=%d',array($id));
		self::add_info($result);
		return $result;
	}
	public static function get_all($start,$end,$order='') {
		$ret = DB::GetAll('SELECT \'blue\' as color,start,end,employee_id,id,0 as timeless FROM crm_projectplanner_work WHERE ((start>=%T AND start<%T) AND project_id=%d)',array($start,$end,self::$project));
		foreach($ret as &$v) {
			self::add_info($v);
		}
		return $ret;
