...
//		print($this->t("Data imported successfully. ").'<a '.$this->create_href(array()).'>'.$this->t("Back").'</a>');
	...
	public function upload_activities($file, $oryginal_file) {
		...
		while($v=fgetcsv($f)) {
			...
			$created_by = $this->get_add_user($v[$header['CREATEUSERID']]);
			if($v[$header['ORGANIZEUSERID']])
				$created_by_contact = CRM_ContactsCommon::get_contact_by_user_id($this->get_add_user($v[$header['ORGANIZEUSERID']]));
			else
			...
			switch($v[$header['ACTIVITY_NAME']]) {
				...
				case 'To-do':
					...
					$rec = array(	'title'=>$title_s,
							'description'=>$desc_s,
							'priority'=>$prio,
							'deadline'=>($end!=null)?date('Y-m-d H:i:s',$end):'',
							'status'=>($stat==0)?0:2,
							'longterm'=>false,
							'permission'=>$access,
							'employees'=>array($created_by_contact),
