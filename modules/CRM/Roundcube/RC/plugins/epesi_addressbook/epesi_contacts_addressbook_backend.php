...
class epesi_contacts_addressbook_backend extends rcube_addressbook
{
  ...
  public function list_records($cols=null, $subset=0, $nocount=false)
  {
    ...
    foreach($fields as $k=>$f) {
        ...
        $queries[] = '(SELECT '.DB::concat('id',DB::qstr('_'.$k)).' as ID, f_first_name as firstname, f_last_name as surname, '.DB::concat('f_last_name',DB::qstr(' '),'f_first_name').' as name, '.$f.' as email FROM contact_data_1 WHERE active=1 AND '.$f.'!=\'\'  AND '.$f.' is not null AND (CAST(f_permission AS decimal)<2 OR created_by='.$E_SESSION['user'].'))';
    }

    $ret = DB::SelectLimit(implode(' UNION ',$queries).' UNION (SELECT '.DB::concat('cd.id',DB::qstr("_-"),'me.id').' as ID, cd.f_first_name as firstname, cd.f_last_name as lastname, '.DB::concat('f_last_name',DB::qstr(' '),'f_first_name').' as name, me.f_email as email FROM contact_data_1 cd INNER JOIN rc_multiple_emails_data_1 me ON (me.f_record_id=cd.id AND me.f_record_type=\'contact\') WHERE cd.active=1 AND me.active=1 AND (CAST(cd.f_permission AS decimal)<2 OR cd.created_by='.$E_SESSION['user'].')) ORDER BY name',$length,$start_row);
  ...
  public function search($fields_to_search, $value, $strict=false, $select=true, $nocount=false, $required=array())
  {
    ...
    if($strict)
        $ret = DB::Execute('SELECT c.id as ID,c.f_first_name as firstname, c.f_last_name as surname, m.f_email as memails, m.id as mid,com.f_company_name'.($m_cols?','.implode(', ',$m_cols):'').' FROM contact_data_1 c LEFT JOIN rc_multiple_emails_data_1 m ON (m.f_record_id=c.id AND m.f_record_type=\'contact\') LEFT JOIN company_data_1 com ON com.id=c.f_company_name WHERE c.active=1 AND (CAST(c.f_permission AS decimal)<2 OR c.created_by=%d) AND ('.($m_cols?implode('='.DB::qstr($vals['email']).' OR ',$m_cols).'='.DB::qstr($vals['email']).' OR ':'').'c.f_first_name=%s OR c.f_last_name=%s OR m.f_email=%s) ORDER BY c.f_last_name,c.f_first_name',array($E_SESSION['user'],$vals['firstname'],$vals['surname'],$vals['email']));
    else
        $ret = DB::Execute('SELECT c.id as ID,c.f_first_name as firstname, c.f_last_name as surname, m.f_email as memails, m.id as mid,com.f_company_name'.($m_cols?','.implode(', ',$m_cols):'').' FROM contact_data_1 c LEFT JOIN rc_multiple_emails_data_1 m ON (m.f_record_id=c.id AND m.f_record_type=\'contact\') LEFT JOIN company_data_1 com ON com.id=c.f_company_name WHERE c.active=1 AND (CAST(c.f_permission AS decimal)<2 OR c.created_by=%d) AND ('.($m_cols?implode(' LIKE '.DB::concat(DB::qstr("%%"),DB::qstr($vals['email']),DB::qstr("%%")).' OR ',$m_cols).' LIKE '.DB::concat(DB::qstr("%%"),DB::qstr($vals['email']),DB::qstr("%%")).' OR ':'').'c.f_first_name LIKE '.DB::concat(DB::qstr("%%"),'%s',DB::qstr("%%")).' OR c.f_last_name LIKE '.DB::concat(DB::qstr("%%"),'%s',DB::qstr("%%")).' OR m.f_email LIKE '.DB::concat(DB::qstr("%%"),'%s',DB::qstr("%%")).' OR com.f_company_name LIKE '.DB::concat(DB::qstr("%%"),'%s',DB::qstr("%%")).') ORDER BY c.f_last_name,c.f_first_name',array($E_SESSION['user'],$vals['firstname'],$vals['surname'],$vals['email'],$vals['name']));
    $done_ids = array();
    while($row = $ret->FetchRow()) {
        if(!isset($row['ID']) && isset($row['id'])) $row['ID'] = $row['id'];
