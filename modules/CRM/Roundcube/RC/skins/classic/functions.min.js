 ...
 */
function rcube_init_settings_tabs(){var a,b;b=$("#tabsbar");var c=$("span:last",b),d="#settingstabpreferences",e=window.rcmail&&rcmail.env.action?rcmail.env.action:null;c&&"settingstababout"!=c.attr("id")&&(a=$("#settingstababout"))&&(b=a.clone(!0),a.remove(),c.after(b));e&&(d="#settingstab"+(0<e.indexOf("identity")?"identities":e.replace(/\./g,"")));$(d).addClass("tablink-selected");$("a",d).removeAttr("onclick").click(function(){return!1})}
...
a),e=d.is(":visible")?-1:1,f=24+($("select",d).length?24:0);d.slideToggle({duration:160,progress:function(b,d){0>e&&(d=1-d);$(".boxlistcontent",a).css("top",c.outerHeight()+f*d+"px")},complete:function(){d.toggleClass("expanded");d.is(":visible")?d.find("input[type=text]").focus():$("a.reset",d).click()}});return!1})}};
...
function fit_string_to_size(a,b,c){var d,e,f=a;rcmail.env.tmp_span?b=rcmail.env.tmp_span:(b=$("<b>").css({visibility:"hidden",padding:"0px","font-family":b.css("font-family"),"font-size":b.css("font-size")}).appendTo($("body",document)).get(0),rcmail.env.tmp_span=b);e=$(b);e.text(f);d=b.offsetWidth;if(d>c){c=Math.max(1,Math.floor((d-c)/d*a.length/2));for(var g=d=f=Math.floor(a.length/2);;){d=f-c;g=f+c;e.text(a.substring(0,d)+"..."+a.substring(g));if(3>d||b.offsetWidth)break;c++}f=a.substring(0,d)+
...
l.addClass("quota_low"));$(a).html("").append(l).append(f).append(k);$("#quotaimg").attr("title",b.title)}var rcmail_editor_settings={},rcmail_ui;
