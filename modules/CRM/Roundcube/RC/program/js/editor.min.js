 ...
 */
function rcube_text_editor(k,m){var h=this,p=location.href.replace(/[?#].*$/,"").replace(/\/$/,""),l={selector:"#"+($("#"+m).is(".mce_editor")?m:"fake-editor-id"),cache_suffix:"s=4010900",theme:"modern",language:k.lang,content_css:rcmail.assets_path("program/js/tinymce/roundcube/content.css"),menubar:!1,statusbar:!1,toolbar_items_size:"small",extended_valid_elements:"font[face|size|color|style],span[id|class|align|style]",relative_urls:!1,remove_script_host:!1,convert_urls:!1,image_description:!1,
paste_webkit_style:"color font-size font-family",paste_data_images:!0,browser_spellcheck:!0};this.spellcheck_observer=function(){};k.spellchecker&&(this.spellchecker=k.spellchecker,k.spellcheck_observer&&(this.spellchecker.spelling_state_observer=this.spellcheck_observer=k.spellcheck_observer));tinymce.registered_request_token||(tinymce.registered_request_token=!0,tinymce.util.XHR.on("beforeSend",function(c){c.xhr.setRequestHeader("X-Roundcube-Request",rcmail.env.request_token)}));"identity"==k.mode?
$.extend(l,{plugins:"autolink charmap code colorpicker hr image link paste tabfocus textcolor",toolbar:"bold italic underline alignleft aligncenter alignright alignjustify | outdent indent charmap hr link unlink image code forecolor | fontselect fontsizeselect",file_browser_callback:function(c,a,d,b){h.file_browser_callback(c,a,d)},file_browser_callback_types:"image"}):$.extend(l,{plugins:"autolink charmap code colorpicker directionality emoticons link image media nonbreaking paste table tabfocus textcolor searchreplace"+
(k.spellcheck?" spellchecker":""),toolbar:"bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent ltr rtl blockquote | forecolor backcolor | fontselect fontsizeselect | link unlink table | emoticons charmap image media | code searchreplace undo redo",spellchecker_rpc_url:p+"/?_task=utils&_action=spell_html&_remote=1",spellchecker_language:rcmail.env.spell_lang,accessibility_focus:!1,file_browser_callback:function(c,a,d,b){h.file_browser_callback(c,a,
d)},file_browser_callback_types:"image media"});window.rcmail_editor_settings&&$.extend(l,window.rcmail_editor_settings);l.setup=function(c){c.on("init",function(a){h.init_callback(a)});c.on("SpellcheckStart SpellcheckEnd",function(a){h.spellcheck_active="spellcheckstart"==a.type;h.spellcheck_observer()});c.on("keypress",function(){rcmail.compose_type_activity++})};this.id=m;this.editor=null;tinymce.init(l);this.init_callback=function(c){this.editor=c.target;if("compose"==rcmail.env.action){var a=
$("#"+this.id),d=$("div.mce-toolbar-grp:first",a.parent()).height();if(200<d||d>a.height())return setTimeout(function(){h.init_callback(c)},300);var a={},d=rcube_find_object("_from"),b=rcmail.env.compose_focus_elem;rcmail.env.default_font&&(a["font-family"]=rcmail.env.default_font);rcmail.env.default_font_size&&(a["font-size"]=rcmail.env.default_font_size);(a["font-family"]||a["font-size"])&&$(this.editor.getBody()).css(a);d&&"select-one"==d.type&&(rcmail.env.identities_initialized||rcmail.change_identity(d),
b&&b.id!=this.id&&(window.focus(),b.focus(),rcmail.env.compose_focus_elem=null));h.tabindex(b&&b.id==h.id);$(window).resize()}};this.tabindex=function(c){if("mail"==rcmail.env.task&&this.editor){var a=this.editor.getElement(),d=this.editor.getBody(),b=this.editor.getContentAreaContainer().childNodes[0];a&&b&&(b.tabIndex=a.tabIndex);if(0<a.tabIndex){var f=null,b=[":prev",":next"],a=tinymce.DOM.select("*[tabindex="+a.tabIndex+"]:not(iframe)");tinymce.each(a,function(a,b){if(a.id==h.id)return f=b,!1});
null!==f&&(a[f-1]&&a[f-1].id&&(b[0]=a[f-1].id),a[f+1]&&a[f+1].id&&(b[1]=a[f+1].id),this.editor.settings.tabfocus_elements=b.join(","))}bw.mz&&25>bw.vendver&&$(d).prop("contenteditable",!1).prop("contenteditable",!0);c&&d.focus()}};this.toggle=function(c,a){var d,b,f=$("#"+this.id),e=rcmail.env.identity?rcmail.env.signatures[rcmail.env.identity]:null,g=e&&e.text&&1<e.text.length;this.spellcheck_stop();if(c)b=f.val(),g&&(b=b.replace(/\r\n/,"\n"),b=b.replace(e.text.replace(/\r\n/,"\n"),"\u0002\u0003")),
d=function(a){g&&(a=a.replace("\u0002\u0003",'<div id="_rc_sig">'+e.html+"</div>"));f.val(a);tinymce.execCommand("mceAddEditor",!1,h.id);h.editor&&(a=$(h.editor.getBody()),h.tabindex(!0),h.editor.selection.setCursorLocation(a.children().first().get(0)))},a?(d(b),b=!0):b=rcmail.plain2html(b,d);else if(this.editor){if(g){if(d=this.editor.dom.get("_rc_sig"))d=d.innerHTML;this.editor.dom.setHTML("_rc_sig","\u0002\u0003")}b=this.editor.getContent();var n=function(a){tinymce.execCommand("mceRemoveEditor",
!1,h.id);h.editor=null;g&&(a=a.replace("\u0002\u0003","\n"+e.text));f.val(a).focus();rcmail.set_caret_pos(f.get(0),0)};a?(n(f.val()),b=!0):b=rcmail.html2plain(b,n);!b&&d&&this.editor.dom.setHTML("_rc_sig",d)}return b};this.spellcheck_start=function(){this.editor?(tinymce.execCommand("mceSpellCheck",!0),this.spellcheck_observer()):this.spellchecker&&this.spellchecker.spellCheck&&this.spellchecker.spellCheck()};this.spellcheck_stop=function(){var c=this.editor;c?c.plugins&&c.plugins.spellchecker&&this.spellcheck_active&&
...
$("div.mce-abs-end",a.getEl()).append($('<div class="hint">').text($("div.hint",rcmail.gui_objects.uploadform).text()));f=$("#image-upload-button").append($("<span>").text(rcmail.gettext("add"+d)));e=f.parents(".mce-panel").find("button:last").parent();f.keydown(function(a){if(9==a.which)return rcube_event.get_modifier(a)==SHIFT_KEY?$("#image-selector-list li:last").focus():e.focus(),!1});e.keydown(function(a){if(9==a.which)return rcube_event.get_modifier(a)==SHIFT_KEY?f.focus():$("#image-selector-list li:first").focus(),
!1});this.hack_file_input(f,rcmail.gui_objects.uploadform);if(window.XMLHttpRequest&&XMLHttpRequest.prototype&&XMLHttpRequest.prototype.sendAsBinary||window.FormData)rcmail.env.filedrop||(rcmail.env.filedrop={}),rcmail.gui_objects.filedrop&&(rcmail.env.old_file_drop=rcmail.gui_objects.filedrop),rcmail.gui_objects.filedrop=$("#image-selector-form"),rcmail.gui_objects.filedrop.addClass("droptarget").bind("dragover dragleave",function(a){a.preventDefault();a.stopPropagation();$(this)["dragover"==a.type?
"addClass":"removeClass"]("hover")}).get(0).addEventListener("drop",function(a){return rcmail.file_dropped(a)},!1);rcmail.env.file_dialog_event||(rcmail.env.file_dialog_event=!0,rcmail.addEventListener("fileuploaded",function(a){if(a=h.file_browser_entry(a.name,a.attachment))$("#image-selector-list > ul").prepend(a),a.focus()}))};this.file_browser_close=function(c){var a=$("#"+rcmail.env.file_browser_field);c&&a.val(c);this.editor.windowManager.close();a.focus();rcmail.env.old_file_drop&&(rcmail.gui_objects.filedrop=
rcmail.env.old_file_drop)};this.file_browser_entry=function(c,a){if(a.complete&&a.mimetype){rcmail.file_upload_id&&rcmail.set_busy(!1,null,rcmail.file_upload_id);var d,b;switch(rcmail.env.file_browser_type){case "image":d=/^image\//i;break;case "media":d=/^video\//i;b="program/js/tinymce/roundcube/video.png";break;default:return}if(d.test(a.mimetype))return d=rcmail.env.comm_path+"&_from="+rcmail.env.action+(rcmail.env.compose_id?"&_id="+rcmail.env.compose_id+"&_action=display-attachment":"&_action=upload-display")+
"&_file="+c,b=$("<img>").attr({title:a.name,src:b?b:d+"&_thumbnail=1"}),$("<li>").attr({tabindex:0}).data("url",d).append($('<span class="img">').append(b)).append($('<span class="name">').text(a.name)).click(function(){h.file_browser_close($(this).data("url"))}).keydown(function(a){if(13==a.which)h.file_browser_close($(this).data("url"));else if(9==a.which)return rcube_event.get_modifier(a)==SHIFT_KEY?$(this).prev().focus().length||$("#image-upload-button").parents(".mce-panel").find("button:last").parent().focus():
$(this).next().focus().length||$("#image-upload-button").focus(),!1})}};this.hack_file_input=function(c,a){function d(a){f.css({top:a.pageY-g.top-10+"px",left:a.pageX-g.left-10+"px"})}var b=$(c),f=$("<input>").attr("name","_file[]"),e=$("<form>").attr({method:"post",enctype:"multipart/form-data"}),g=b.offset();a&&(f.attr("name",$('input[type="file"]',a).attr("name")),e.attr("action",$(a).attr("action")).append($("<input>").attr({type:"hidden",name:"_token",value:rcmail.env.request_token})));f.attr({type:"file",
multiple:"multiple",size:5,title:"",tabindex:-1}).change(function(){rcmail.upload_file(e,"upload")}).click(function(){setTimeout(function(){b.mouseleave()},20)}).css({opacity:0,cursor:"pointer",position:"relative",outline:"none"}).appendTo(e);navigator.userAgent.match(/Firefox|MSIE/)&&f.css({marginLeft:"-80px"});b.css({overflow:"hidden",cursor:"pointer"}).mouseenter(function(){this.__active=!0}).mousemove(function(a){this.__active?d(a):$(this).mouseleave()}).mouseleave(function(){f.css({top:"-10000px",
left:"-10000px"});this.__active=!1}).click(function(a){this.__active||(this.__active=!0,d(a),f.trigger(a))}).keydown(function(a){13==a.which&&f.trigger("click")}).mouseleave().append(e)}};
