{* CRM_PhoneCall registers this as the per-table template for the
   ...
   as View_entry.tpl/Contact.tpl; tooltips row kept. Layout below (the two-
   column subject/customer/phone vs date/status/priority split, multiselects,
   longfields) mirrors View_entry.tpl/Contact.tpl's own conversion to flex
   (.epesi-rv-columns/.column/.view/.edit/.epesi-rv-row/.label/.data instead
   of a <table> of <table>s) - real per-field content, not decoration,
   unchanged beyond the wrapper markup. id="CRM_PhoneCall" is kept on the
   wrapper (unlike Contact.tpl, this module's own default theme CSS -
   modules/CRM/PhoneCall/theme/default.css - scopes status/priority/
   permission dropdown icon backgrounds off this id; no theme_adminlte
   override exists for that file, so it still resolves via the normal
   fallback-to-default mechanism and needs the id present to keep matching).
