...
class Tests_Calendar_EventCommon extends Utils_Calendar_EventCommon {
	public static function get_all($start,$end) {
		return DB::GetAll('SELECT \'\' as additional_info,color,start,duration,title,description,id,timeless FROM tests_calendar_event WHERE ((start>=%d AND start<%d))',array($start,$end));
	}

	public static function get($id) {
		return DB::GetRow('SELECT \'\' as additional_info,color,start,duration,title,description,id,timeless FROM tests_calendar_event WHERE id=%d',array($id));
	}
