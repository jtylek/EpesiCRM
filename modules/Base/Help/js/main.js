Base_Help = function(){
	...
	this.operation;
	this.help_hooks;
	this.click_icon = document.getElementById("Base_Help__click_icon");
	this.help_arrow = document.getElementById("Base_Help__help_arrow");
	this.comment_frame = document.getElementById("Base_Help__help_comment");
	this.screen = '';
	...
	this.init_help = function() {
		...
		Helper.timed_update();
		jQuery(document).on("mousemove", Helper.update);
	}
	...
	this.check_compatibility = function() {
		...
		if (has_pointerevents=='auto') {
			...
			this.context = document.getElementById("help_canvas").getContext("2d");
	...
	this.stop_tutorial = function() {
        jQuery("#help_canvas").hide();
		document.getElementById('Base_Help__overlay').style.display = 'none';
	...
	this.refresh_step = function() {
		...
		if (this.target) jQuery(this.target).on('click', function(){Helper.prompt_next_step = true;});
	...
	this.update = function(e) {
		current = new Date().getTime();
		if (this.trigger_search && (current - this.last_keypress)>800) this.search(document.getElementById('Base_Help__search').value);
	...
	this.escape = function() {
		if (document.getElementById('Base_Help__menu').style.display=="block") this.hide_menu();
	...
	this.menu = function () {
		this.stop_tutorial();
		document.getElementById('Base_Help__overlay').style.display="block";
		document.getElementById('Base_Help__menu').style.display="block";
		document.getElementById('Base_Help__search').value='';
	...
	this.search = function(value) {
		this.trigger_search = false;
		if (!value) {
			document.getElementById('Base_Help__help_suggestions').style.display='block';
			document.getElementById('Base_Help__help_links').style.display='none';
			jQuery.ajax('modules/Base/Help/suggestions.php', {
				method: 'post',
				data:{
					cid: Epesi.client_id
				},
			...
			document.getElementById('Base_Help__help_suggestions').style.display='none';
			document.getElementById('Base_Help__help_links').style.display='block';
			jQuery.ajax('modules/Base/Help/search.php', {
				method: 'post',
				data:{
					cid: Epesi.client_id,
					keywords: value
	...
	this.hide_menu = function () {
		document.getElementById('Base_Help__overlay').style.display="none";
		document.getElementById('Base_Help__menu').style.display="none";
	}
	...
	this.get_all_help_hooks = function() {
		document.getElementById('Base_Help__button_next').onclick = function(){Helper.prompt_next_step = true;};
		document.getElementById('Base_Help__button_finish').onclick = function(){Helper.prompt_next_step = true;};
	...
	this.draw_help_arrow = function (el) {
		...
		if (this.operation=='finish') {
			document.getElementById('Base_Help__overlay').style.display = 'block';
			document.getElementById('Base_Help__button_finish').style.display = 'block';
			document.getElementById('Base_Help__button_next').style.display = 'none';
			this.help_arrow.style.display = "none";
			...
			document.getElementById('Base_Help__button_next').style.display = 'block';
			document.getElementById('Base_Help__button_finish').style.display = 'none';
			targetX = o_right;
			targetY = o_bottom;
			sourceX = targetX+50;
			sourceY = targetY+100;
		...
		if (this.steps[this.step].comment && (this.operation!='fill' || this.target==document.activeElement)) {
			document.getElementById('Base_Help__help_comment_contents').innerHTML = this.steps[this.step].comment;
