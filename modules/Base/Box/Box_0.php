...
//			else $containers = $ini_file['Anonymous'];
		...
		foreach ($containers as $k => $v) {
			...
			if(ModuleManager::is_installed($v['module'])==-1) {
				print($lang->t("Please install %s module or choose another theme!",$v['module'])."<br><a ".$this->create_href(array('box_main_module'=>'Setup')).">".$lang->t('Manage modules').'</a><br><a '.$this->create_href(array('box_main_module'=>'Base/Theme/Administrator')).'>'.$lang->t("Choose another theme").'</a>');
			} else {
				if(isset($v['function']))
