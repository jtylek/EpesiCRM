...
class Base_Theme extends Module {
	...
	private function load_css_cache() {
		...
		if(load_css(self::$themes_dir.self::$theme.'/__cache.css', self::$themes_dir.self::$theme.'/__css.php?f=')) {
			$arr = explode("\n",file_get_contents(self::$themes_dir.self::$theme.'/__cache.files'));
			foreach($arr as $f)
				$_SESSION['client']['__loaded_csses__'][$f] = 1;
		}
		if(self::$theme!='default' && load_css(self::$themes_dir.self::$theme.'/__cache.css', self::$themes_dir.'default/__css.php?f=')) {
